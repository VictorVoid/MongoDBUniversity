 ...
 */
        ...
        var skip = page > 0 ? (page*itemsPerPage) : 0;
            console.log(category);
            console.log(query);
        this.db.collection('item').find(query).skip(skip).limit(itemsPerPage).toArray(function(err, docs) {
            console.log(docs);
            callback(docs);
        })
    }
 ...
 */     var regex = new RegExp(query, "i");
        var qry = {title: {$regex: regex}};
        var skip = page > 0 ? (page*itemsPerPage) : 0;

        this.db.collection('item').find(qry).skip(skip).limit(itemsPerPage).toArray(function(err, docs){
        assert.equal(err, null);    
    ...
    this.getNumSearchItems = function(query, callback) {
        ...
        */
        ...
        this.db.collection('item').find(qry).toArray(function(err, docs){
            assert.equal(err, null);
...
this.addReview = function(itemId, comment, name, stars, callback) {
        ...
        var query = {$push: {reviews: reviewDoc}};
        console.log(query);
        this.db.collection("item").updateOne({_id: itemId}, query);
        this.db.collection("Item").find({_id: itemId}, function(err, docs) {
            assert.equal(null, err);
            callback(docs);
        })
        ...
        this.db.collection('item').updateOne({'_id': itemId}, { $push: { 'reviews': reviewDoc } }, { upsert: true }, callback(this))*/
    }
