...
function CartDAO(database) {
    ...
    this.getCart = function(userId, callback) {
        ...
        */
        ...
        this.db.collection('cart').findOne(query, function(err, docs) {
    ...
    this.itemInCart = function(userId, itemId, callback) {
        "use strict";
         /*
          var query = {$and:[{userId:  "558098a65133816958968d88"}, {"items.id": 2}]}
          var query = {moves: {$in: [/veloz demais/i]}}
          Usando o elemMatch
          db.schools.find( { zipcode: "63109" },
          { students: { $elemMatch: { school: 102 } } } )
          { students: { $elemMatch: { school: 102 } } } )
        var query = {userId:"558098a65133816958968d88"},{items:{$elemMatch:{_id: 2}}};*/
                       //      558098a65133816958968d88
        //CORRETO --> var query = {$and:[{userId:"558098a65133816958968d88"},{items:{$elemMatch:{_id: 2}}}]}
        var encontrou = false;
        var query = {$and:[{userId:userId},{items:{$elemMatch:{_id: itemId}}}]}
        this.db.collection('cart').findOne(query, function(err, docs){
            assert.equal(err, null);
            if (docs != null){
                console.log("ENCONTROU !!!!!"+docs);
                encontrou = true;
            }else {
        ...
        if (encontrou){
            this.db.collection('item').findOne({_id: itemId}, function(err, docs){
                assert.equal(err, null);
                console.log("Encontrou O ITEM OLHA: "+docs);
                callback(docs);
            });
        }else {
            callback(null);
            console.log("NAO ENCONTROU O ITEM");
        }
        /*
         *
         * TODO-lab6
