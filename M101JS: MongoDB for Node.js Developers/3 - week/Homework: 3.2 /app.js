...
MongoClient.connect('mongodb://localhost:27017/be-mean', function(err, db){
	...
	var cursor = db.collection('schoolgrade').find({}).sort({"grade": 1})
	.skip(6).limit(2).toArray(function(err, doc){
		console.log(doc);
	})
});
/* RESULTADO:  
...
Suppose you have a MongoDB collection called school.grades that is
composed solely of these 20 documents:

Assuming the variable db holds a connection to the school
database in the following code snippet.
...
*/
var query = {student:"David"}
var mod = {$set:
         {
           grade: 5
         }
     }
db.schoolgrade.update(query,mod)
